...
window.addEventListener("DOMContentLoaded", () => {
    ...
    $(".decision__presentation-items").slick({
        ...
        nextArrow:
            '<button type="button" class="slick-next"><img src="img/decision/decision_arrow_right.svg"></button>',
    ...
    function openContacts() {
        ...
        contactsHeader.addEventListener("click", (event) => {
            ...
            if (
                target.classList.contains("subheader__text-name") ||
                target.classList.contains("country_contacts") ||
                target.parentNode.classList.contains("subheader__item-flag")
            ) {
                for (i = 0; i < contactsItem.length; i++) {
                    if (target == contactsMenu[i] || target == contactsBtn[i] || target == contactsFlag[i]) {
                        contactsMenu[i].classList.toggle("btn-active");
                        contactsItem[i].classList.toggle("country-active");
                    }
                }
            }
        });
